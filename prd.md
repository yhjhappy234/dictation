...
- **预设内容分类**：
  ...
  **教材类（支持小学/初中/高中）**：
  ...
  - 初中语文教科书（七至九年级上下册）
  - 高中语文教科书（高一至高三上下册）
  - 选择流程：点击分类 → 选择年级 → 选择单元 → 填充词语

  **常用词语分类**：
  - 水果、天气、颜色、动物、蔬菜、食物、植物
  - 职业、交通工具、服装、身体部位、时间、地点
  - 家庭称呼、学校用品（每类约30个词语）
  ...
  **常用古文分类**：
  - 修身篇、学习篇、自然篇、道德篇、家训篇、劝学篇

- **导入流程**：
  - 点击对应分类按钮 → 选择子分类 → 自动打开创建面板 → 填充词语到输入框 → 填充任务名称 → 用户点击"保存任务"完成创建
- **提示信息**：导入后显示"已填充：xxx，请点击'保存任务'完成创建"
- **数据存储**：预设内容以JSON格式存储于resources/preset-content目录

#### 2.1.3 自动听写功能（新增）
- **功能描述**：自动定时执行听写，无需手动点击下一个
- **自动听写设置**：
  - 默认间隔：3秒
  - 可设置范围：1-60秒
  - 设置保存于localStorage
- **操作流程**：
  - 点击"自动听写"按钮(A键快捷键) → 自动播放当前词语 → 定时后自动执行下一个词语并标记为正确 → 循环直到完成
  - 再次点击按钮或按A键可停止自动听写
- **实时统计显示**：
  - 当前听写耗时（秒数实时更新）
  - 正确听写个数
...
- **任务状态**：未开始、进行中、已完成
- **进度显示**：
  - 进行中任务：显示正确/错误数、进度百分比
  - 已完成任务：显示100%进度
- **任务操作**：创建、编辑、删除、开始、完成、重置、收藏
...
- **v1.3 (2026-04-18)**:
  - 自动听写功能：定时自动执行下一个词语，可设置间隔(1-60秒)
  - 实时统计显示：耗时、正确/错误数、进度百分比
  - 预设内容分类优化：
    - 常用词语15个分类（水果、天气、颜色、动物等）
    - 成语8个分类（数字成语、自然景色、人物描写等）
    - 古诗8个分类（春季、夏季、秋季、山水等）
    - 古文6个分类（修身篇、学习篇、自然篇等）
  - 教材扩展：
    - 初中语文教科书（七至九年级上下册）
    - 高中语文教科书（高一至高三上下册）
  - 任务管理模块：进行中任务显示正确/错误数和进度百分比
  - Flyway数据库版本管理
